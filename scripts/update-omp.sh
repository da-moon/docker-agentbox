...
readonly package_file="${repo_root}/nix/packages/omp.nix"
readonly releases_api="https://api.github.com/repos/can1357/oh-my-pi/releases/latest"
...
usage() {
  cat <<'EOF'
Usage: scripts/update-omp.sh [OPTIONS]
...
[ -n "$current_version" ] || {
  log_error "Could not read the current omp version"
  exit 2
}
...
if [ "$check_only" = true ]; then
  if [ "$current_version" = "$latest_version" ]; then
    log_info "omp is current (${current_version})"
    exit 0
  fi
  log_warn "omp update available: ${current_version} -> ${latest_version}"
  exit 1
fi

if [ "$current_version" = "$latest_version" ] && [ "$rehash" = false ]; then
  log_info "omp is already at ${current_version}"
  exit 0
fi

x86_url="https://github.com/can1357/oh-my-pi/releases/download/v${latest_version}/omp-linux-x64"
arm_url="https://github.com/can1357/oh-my-pi/releases/download/v${latest_version}/omp-linux-arm64"
...
if [ "$no_build" = false ]; then
  verify_package_build "$repo_root" omp
fi

committed=true
log_info "Updated omp: ${current_version} -> ${latest_version}"
